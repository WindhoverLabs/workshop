...
export YAMCS_CFS_VERSION=1.0.4-beta
export ECLIPSE_VERSION=oxygen-M7-linux-gtk-x86_64
export YAMCS_STUDIO_VERSION=1.5.2
...
sudo wget https://github.com/yamcs/yamcs-studio/releases/download/v${YAMCS_STUDIO_VERSION}/yamcs-studio-${YAMCS_STUDIO_VERSION}-linux.gtk.x86_64.tar.gz
sudo tar -xzf yamcs-studio-${YAMCS_STUDIO_VERSION}-linux.gtk.x86_64.tar.gz
sudo mv yamcs-studio-${YAMCS_STUDIO_VERSION} /opt/yamcs-studio
sudo chown -R root:root /opt/yamcs-studio
sudo rm yamcs-studio-${YAMCS_STUDIO_VERSION}-linux.gtk.x86_64.tar.gz

cd ${WORKSPACE}
sudo wget https://github.com/WindhoverLabs/yamcs-cfs/releases/download/${YAMCS_CFS_VERSION}/yamcs-cfs-${YAMCS_CFS_VERSION}.jar
sudo cp yamcs-cfs-${YAMCS_CFS_VERSION}.jar /opt/yamcs/lib
sudo chown -R root:root /opt/yamcs/lib
sudo rm yamcs-cfs-${YAMCS_CFS_VERSION}.jar



#########################################################
#Has been tested only up until this point 
#TODO: Test the rest of the srcipt on Ubuntu20
#########################################################
...
sudo apt-get install -y python-pip python-dev bu
# Install Eclipse
wget http://jenkins.windhoverlabs.lan/external-packages/eclipse/eclipse-cpp-${ECLIPSE_VERSION}.tar.gz
echo "WORKSHOP_BASEDIR = $WORKSHOP_BASEDIR"
tar -xzf eclipse-cpp-${ECLIPSE_VERSION}.tar.gz
...
sudo apt-get install -y libc6-dbg libc6-dbg:i386ild-essential
sudo -H pip install --upgrade pip
sudo -H pip install --upgrade virtualenv
sudo -H pip install catkin_pkg
sudo sh -c 'echo "deb http://packages.osrfoundation.org/gazebo/ubuntu-stable `lsb_release -cs` main" > /etc/apt/sources.list.d/gazebo-stable.list'
